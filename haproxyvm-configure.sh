...
# configure DNS server here, only neccessary for dynamic hostnames that change
# the DNS server needs to be valid, this is an AWS DNS
...
# the resolvers config is just needed if the hostnames change IP frequently
listen atc-bmwgroup-net
    bind 0.0.0.0:443
    mode tcp
    timeout connect 4000
    timeout client 180000
    timeout server 180000
    server srv2 atc.bmwgroup.net:443 

# the resolvers config is just needed if the hostnames change IP frequently    
listen git-bmwgroup-net
    bind 0.0.0.0:7999
    mode tcp
    timeout connect 4000
    timeout client 180000
    timeout server 180000
    server srv3 git.bmwgroup.net:7999" > $HAPROXY_CFG
